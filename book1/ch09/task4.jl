...
Write a function called `isabecedarian` that returns `true` if the letters in a
...
# ╔═╡ 95ea71dd-bb49-48a6-b551-322cda1a18d1
function isabecedarian(word::String)::Bool
	l_case_word::String = lowercase(word)
	join(sort(split(l_case_word, "")), "") == l_case_word
end

# ╔═╡ ff45c945-43d0-4159-9953-2b01481251ff
function avoids(word::String, to_avoid::String)::Bool
	for c in to_avoid
		if c in word
			return false
		end
...
with_terminal() do
	...
	println("$(length(abecedarians)) abecedarians / $(length(all_words)) words")
	println("Printing abecedarians.\n")
	for word in abecedarians
		println(word)
	end
	println("\nThat's all. Goodbye!")
end
