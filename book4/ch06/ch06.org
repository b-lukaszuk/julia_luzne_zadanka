...
function addDist(pmf1::Pmf{Int}, pmf2::Pmf{Int})::Pmf{Int}
    ...
    newName::Int = 0
    newPrior::Flt = 0.0
    for (n1, n2) in getCartesianProduct(pmf1.names, pmf2.names)
        for (p1, p2) in getCartesianProduct(pmf1.priors, pmf2.priors)
            newName = n1 + n2
            newPrior = p1 * p2
            result[newName] = get(result, newName, 0) + newPrior
        end
    end
    sortedNames::Vec{Int} = sort(c(keys(result)))
    sortedPriors::Vec{Flt} = [result[n] for n in sortedNames]
    return Pmf(sortedNames, sortedPriors)
end
