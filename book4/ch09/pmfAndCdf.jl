...
"""
returns a Poisson Pmf

# Arguments
- lam::Num: avg number of successes for Poisson distribution
- qs::Vector{Num}: observed number of successes

# Returns
- Pmf(observed number of successes, probs of those successes under Pois(lam))
"""
function getPoisPmf(
        lam::A,
        qs::Vector{B}
...
"""
updates a Poisson Pmf with new data
# Arguments
- pmfPoisson: Vector{Float64} - names: possible lambdas for Poisson distribution
                                priors: probs of those possible lambdas
- data: Num - k of successes that occured

# Returns
Pmf(possible lambdas for Pois distr, priors) + updated likelihood given data
"""
function updatePoisson!(pmfPoisson::Pmf{Float64}, data::Num)
    ...
    pmfPoisson.likelihoods .*= likelihoods
...
"""
get probability that a random name of pmf1 will be > than a random name of pmf2

# Arguments
- pmf1: Pmf{Num}
- pmf: Pmf{Num}
- usePriors: Bool - should use priors (or posteriors) to calculate the prob
"""
...
"""
get Pdf for exponential distribution
(prob of seeing an interval t betw. two events of the pois distribution)

# Arguments
- t: Num - time between two events in the poisson distribution
- lam: Num - lambda (avg) of success in the poisson distribution
- usePriors: Bool - should use priors (or posteriors) to calculate the prob
"""
...
"""
updates the likelihoods of lams (pmf.names) based on t (time betw events)
"""
function updateExponential!(pmf::Pmf{A}, data::B) where {A<:Num, B<:Num}
    pmf.likelihoods .*= getExpoPdf.(data, pmf.names)
    updatePosteriors!(pmf, true)
    return nothing
end
...
# Arguments
- lam: Num - lambda (avg) of success/event in poisson distribution
- high: Num - upper bound on the iterval t between the events of the Pois distr.

# Returns
Pmf of the possible intervals (0:high)
    Pmf.names tiem between events for a Poisson distribution of given lam
"""
