...
Write a function that reads the file `words.txt` and builds an array with one
element per word. Write two versions of this function, one using `push!` and the
...
function get_lines_from_file1(file_path::String = "./words/words.txt")::Vector{String}
    lines::Vector{String} = []
...
# ╔═╡ 2d30ba51-9c20-4000-9ae3-0ca6a6482dfa
function get_lines_from_file2(file_path::String = "./words/words.txt")::Vector{String}
    lines::Vector{String} = []
    open(file_path) do file
        for line in eachline(file)
            lines = [lines..., line]
...
# ╔═╡ 90d426a0-e99e-45d4-b516-f539ed1956f6
with_terminal() do
	println("Testing get_lines_from_file1 function.")
	println("The function uses 'push!' function")
	@time get_lines_from_file1("./words/words.txt");
end
...
The push! version takes around 0.009 secs, whereas the t = [t..., x] takes around 87.7 secs.

The splat operator (...) copies old array and that's what slows the second function down."""
