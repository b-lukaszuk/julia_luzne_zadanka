...
Just a description of what was done here. There is also a promise that in the
next chapter we will make the leap from using Bayes’s Theorem to doing Bayesian
statistics.

* 3.8. Exercises
** 3.8.1. Exercise 1

There is a box with a 6-, 8-, and a 12-sided die. You choose a dice at random,
roll it four times, and get 1, 3, 5, and 7. What are the posteriors for the
dice?

#+BEGIN_SRC

dice = getPmfFromSeq([6, 8, 12])
updatePosteriors!(dice, [1, 3, 5, 7]) |> pmf2df
...
You choose a dice at random, roll it. get 7. What are the posteriors for the
dice?

#+BEGIN_SRC

dice = getPmfFromSeq(rep([4, 6, 8, 12, 20], c(1:5)))
updatePosteriors!(dice, 7) |> pmf2df
...
- equal ratio of black/white socks,
- equal ratio of red/green/blue socks,

You choose a drawer at random, choose two socks at random. You got a matching
pair. What is the probability that the socks are white (assume drawing with
replacement)?

#+BEGIN_SRC

drawers = Pmf(["BW", "RGB"], [0.5, 0.5])

# for BW: WW or BB (2) out of 2*2 = 4 options
# for RGB: RR, BB, GG (3) out of 3*3 = 9 options
# so, prob of having a matching pair out of a drawer
setLikelihoods!(drawers, [2/4, 3/9])
bayesUpdate!(drawers, true) |> pmf2df

# prob of BW drawer is 0.6 AND prob of getting WW out of WW/BB is 1/2
# 2 independent probabilities, so
pmf2df(drawers)[1, 4] * 1/2

#+END_SRC

** 3.8.4. Exercise 4

A problem from [[https://sites.stat.columbia.edu/gelman/book/][Bayesian Data Analysis]]
...
# P(H) - type of twin
twins = Pmf(["fraternal", "identical"], [2/3, 1/3])

# P(H|D) - twin type | brother brother
# fraternal can be mixed or the same genders, BB is 1 out of 2*2=4 options
# P(fraternal|BB) = 1/4
# identical can be only the same sex, so BB out of BB/GG options, so 1/2
# P(identical|BB) = 1/2
twins.likelihoods = [1/4, 1/2]
bayesUpdate!(twins, true) |> pmf2df
