...
[words.txt](https://github.com/BenLauwens/ThinkJulia.jl/blob/master/data/words.txt)
and prints only the words with more than 20 characters (not counting
whitespace).
...
function print_words_longer_than_n(words::Vector{String}, n::Integer = 20)
	for word in words
        if length(strip(word)) > n
            println(word)
        end
    end
    return nothing
...
with_terminal() do
	println("Printing words from words.txt that are > 20 characters long.\n")
	print_words_longer_than_n(get_lines())
	println("\nThat's all. Goodbye!")
end
