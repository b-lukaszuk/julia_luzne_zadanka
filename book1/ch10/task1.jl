...
function nestedsum(arr, acc::Number = 0)::Number
	if isempty(arr)
		return acc
	elseif isa(arr[1], Array)
		return nestedsum(arr[2:end], acc+nestedsum(arr[1]))
	else
		return nestedsum(arr[2:end], acc+arr[1])
	end
end
...
with_terminal() do
	arr1 = [[1, 2], [3], [4, 5, 6]]
	arr2 = [[1, 2, [1, 2, 3], 2], [4, 5, 6], 7]
	arr3 = [1 2 3; 4 5 6; 8 9 7]
	println("Testing nestedsum function.\n")
	println("nestedsum($arr1) = $(nestedsum(arr1))")
	println("nestedsum($arr2) = $(nestedsum(arr2))")
	println("nestedsum($arr3) = $(nestedsum(arr3))")
	println("\nThat's all. Goodbye!")
end
