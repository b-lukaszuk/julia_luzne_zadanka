...
Write a function called `issort` that takes an array as a parameter and returns
`true` if the array is sorted in ascending order and `false` otherwise. For
example:
...
with_terminal() do
	arr1::Vector{<:Number} = [1, 2, 2]
	arr2::Vector{Char} = ['b', 'a']
	arr3::Vector{String} = ["ala", "as", "tomek"]
	println("Testing issort function\n")
	println("issort($arr1) = $(issort(arr1))")
	println("issort($arr2) = $(issort(arr2))")
	println("issort($arr3) = $(issort(arr3))")
	println("\nThat's all. Goodbye!")
end
