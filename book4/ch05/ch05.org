...
The following priors are based on [[https://www.pewresearch.org/social-trends/2015/05/07/family-size-among-mothers/][this article from the Pew Research Center]].

#+BEGIN_SRC

family = Pmf([1, 2, 3, 4], [22, 41, 24, 14])

Cmk.barplot(family.names, family.priors,
            axis=(; title="Distribution of family size", ylabel="PMF",
                  xticks=(1:4, ["1 child", "2 children", "3 children", "4+ children"])
                  )
            )

#+END_SRC

For simplicity, let’s assume that all families in the 4+ category have exactly 4
children.

*** Solution

Per 100 mothers in generation 1, the next generation (generation 2) got
the expected counts (one may choose some big round number of mothers to avoid
floats in the result):

- 100 * 0.218 * 1 children
- 100 * 0.406 * 2 children
- 100 * 0.238 * 3 children
- 100 * 0.139 * 4 children

So, we got n children:

$n = \sum_{i=1}^{4} 100 * P(children_i) * i_{children} \approx 229.7$

Out of this `n` children only $100 * 0.218 * 1 \approx 22.8$ (so $22.8/229.7 \approx
0.0949$) has no sibling.

So we can calculate the posteriors (only the probabilities) as:

#+BEGIN_SRC

family.posteriors = family.names .* family.priors
normalizePosteriors!(family)

#+END_SRC

Therefore, the probability that a randomly chosen US adult got a sibling is:

#+BEGIN_SRC

sum(family.posteriors[2:end])
...
getPosteriorGT(family, 1)

#+END_SRC

** 5.8.5. Exercise 5

[[https://en.wikipedia.org/wiki/Doomsday_argument][Doomsday argument]]

Suppose there are only two kinds of intelligent civilizations:

- “short-lived” - go extinct after only 200 billion individuals born,
- “long-lived” kind survive until 2,000 billion individuals are born.

Suppose both are equally likely. Which kind of civilization do you think we live
in?

According to the [[https://www.prb.org/articles/how-many-people-have-ever-lived-on-earth/][Population Reference Bureau]] the total number of people who have
ever lived is about 108 billion.

Assume that you are, in fact, human being number 108 billion.

If `N` is the total number who will ever live and we consider you to be a
randomly-chosen person, it is equally likely that you could have been person 1,
or N, or any number in between. So what is the probability that you would be
number 108 billion?
...
# 200_000_000_000 (two hundred billion individuals)
# 2_000_000_000_000 (two thousand billion individuals)
# billion 10^9
doom = getPmfFromSeq([200e9, 2000e9])

#+END_SRC

Probability of being number 108 billion, if being each number (1..N) is equally
likely:

#+BEGIN_SRC

setLikelihoods!(doom, 1 ./ doom.names)

#+END_SRC

Probability that our civilization is short-lived (under the dubious prior)?

#+BEGIN_SRC

bayesUpdate!(doom, true)
