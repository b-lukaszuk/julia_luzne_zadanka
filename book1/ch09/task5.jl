...
Talk (https://www.cartalk.com/puzzler/browse):
...
c-o-m-m-i-t-t-e-e. It would be great except for the i that sneaks in there. Or
Mississippi: M-i-s-s-i-s-s-i-p-p-i. If you could take out those i’s it would
...
function get_lines_from_file(
    file_path::String = "./words/words.txt")::Vector{String}
    lines::Vector{String} = []
...
# ╔═╡ b411753c-3df7-4e87-8c3a-8a11e3edf109
function get_regex_n_double_chars(n::Int)::String
	regex::String = ""
	for i in 1:n
		regex = regex * "([a-z])\\" * string(i)
	end
	regex
end
...
with_terminal() do
	println("Printing words from 'words.txt' with tripple double chars.\n")
	# the file contains 1 word per line
	all_words::Vector{String} = get_lines_from_file()
	words_3x_double_chars::Vector{String} = filter(
        w -> contains_n_double_chars(w,3), all_words)
	for word in words_3x_double_chars
		println(word)
	end
	println("\nThat's all. Goodbye!")
end
