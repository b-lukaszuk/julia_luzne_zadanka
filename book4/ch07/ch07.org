...
To compute the distribution of a maximum or minimum, we'll use a `Cdf`.

#+BEGIN_SRC

cdfBest3of4 = pmf2cdf(pmfBest3of4)

#+END_SRC

`Cdf(x)` is a probability that a quantity is $\le x$ or equivalently that a random
value chosen from the distribution is $\le x$.

If we draw 6 values of the distribution then the probability that all 6 of them
are $\le x$ is `Cdf(x)` raised to the 6th power:

#+BEGIN_SRC

cdfBest3of4.probabilities .^ 6

#+END_SRC

If all 6 values are $\le x$ that means that their maximum is $\le x$, we can express
it as a `Cdf`:

#+BEGIN_SRC

cdfMax6 = Cdf(copy(cdfBest3of4.names), cdfBest3of4.probabilities .^ 6)

#+END_SRC

The equivalent `Pmf` is:

#+BEGIN_SRC

pmfMax6 = cdf2Pmf(cdfMax6)
Cmk.lines(pmfMax6.names, pmfMax6.priors, color=:blue, label="max of 6 attributes",
          axis=(;title="Distribution of attributes", xlabel="Outcome", ylabel="PMF",
                xticks=4:2:18, yticks=0:0.05:0.25)
          );
Cmk.axislegend(position=:lt);
Cmk.current_figure()
...
Below, the comparison of the three `Cdf`s we have computed so far.

#+BEGIN_SRC

cdf3die = pmf2cdf(pmf3die)
Cmk.lines(cdf3die.names, cdf3die.probabilities, color=:blue, label="sum of 3 dice",
          axis=(;title="Distribution of attributes", xlabel="Outcome", ylabel="CDF",
                xticks=4:2:18, yticks=0:0.2:1));
Cmk.lines!(cdfBest3of4.names, cdfBest3of4.probabilities, color=:orange,
           linestyle=:dash, label="best 3 of 4 dice");
Cmk.lines!(cdfMax6.names, cdfMax6.probabilities, color=:green,
           linestyle=:dot, label="max of 6 attributes");
Cmk.axislegend(position=:lt);
Cmk.current_figure()
...
Previously we computed the distribution of a character's best attribute. Now,
let's compute the distribution of the worst.

First, we compute the *complementary CDF*, i.e. the probability that a value from
the distribution is greater than `x`.

#+BEGIN_SRC

probGt = 1 .- cdfBest3of4.probabilities

#+END_SRC

For instance, the probability that all 6 exceed `x` is:
...
complementary CDF of the minimum:
...
Let's compare it with the maximum:

#+BEGIN_SRC

Cmk.lines(cdfMin6.names, cdfMin6.probabilities,
          color=:purple, label="minimum of 6",
          axis=(;title="Minimum and maximum of six attributes",
                xlabel="Outcome", ylabel="CDF", xticks=4:2:18, yticks=0:0.2:1)
          );
Cmk.lines!(cdfMax6.names, cdfMax6.probabilities,
          color=:green, linestyle=:dot, label="maximum of 6",
          );
Cmk.axislegend(position=:lt);
Cmk.current_figure()
