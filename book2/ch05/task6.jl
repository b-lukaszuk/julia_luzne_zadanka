...
Getting the input from the user is boring (besides Pluto's nootebooks are interactive) so I will skip that part. I will also limit myself to some smaller number (it is boring to write down so many numbers on the screen and then read it to check the corectness of the input).
"""
...
function getPrimes(upto::Int = 100)
    @assert 1 < upto <= 1000 "upto needs to be between 2 and 1000"
    isPrime::Vector{Bool} = ones(Bool, upto); isPrime[1] = false
    for i in 2:upto
	for j in (i+i):i:upto
	    isPrime[j] = false
	end
    end
    return collect(1:upto)[isPrime]
end

# ╔═╡ 24b1ea64-9183-4420-be54-0219d2b87216
with_terminal() do
    upperLimit::Int = 30
    println("Testing getPrimes function.")
    println("-" ^ 3)
    println("printing primes upto $upperLimit:")
    foreach(x -> println(x), getPrimes(upperLimit))
    println("-" ^ 3)
end
